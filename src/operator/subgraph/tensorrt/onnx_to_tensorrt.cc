...
std::tuple<unique_ptr<nvinfer1::ICudaEngine>,
  ...
  if (calibrator != nullptr && calibrator->isCacheEmpty()) {
    future_int8_engine = std::async([trt_builder = std::move(trt_builder),
      ...
      auto trt_int8_calibrated_engine = InferObject(trt_builder->buildEngineWithConfig(*trt_network,
                                                                                       *builder_config));
      return std::move(trt_int8_calibrated_engine);
    });
  }
#else
  if (fp16_mode) {
  ...
  if (calibrator != nullptr && calibrator->isCacheEmpty()) {
    future_int8_engine = std::async([trt_builder = std::move(trt_builder),
      ...
      // Calibration is blocking so we need to have it in a different thread.
      // The engine will be calling calibrator.setBatch until it returns false
      auto trt_int8_calibrated_engine = InferObject(trt_builder->buildCudaEngine(*trt_network));
      return std::move(trt_int8_calibrated_engine);
    });
  }
#endif
  return std::make_tuple(std::move(trt_engine), std::move(trt_parser), std::move(trt_logger), std::move(future_int8_engine));
}
