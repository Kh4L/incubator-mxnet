...
class TensorrtProperty : public SubgraphProperty {
 public:
  ...
  void PrePartition(const nnvm::Graph& g,
    ...
    auto it_fp16 = options_map.find("fp16_mode");
    fp16_mode_ = it_fp16 != options_map.end() && it_fp16->second == "True";
    auto it_int8 = options_map.find("int8_mode");
    int8_mode_ = it_int8 != options_map.end() && it_int8->second == "True";
    if (int8_mode_) {
