 ...
 ***************************************************************************************************
 ***************************************************************************************************
 ************ Testing 2-step mask target generation: rasterizing full mask -> roi align ************
 ***************************************************************************************************
 ***************************************************************************************************
*/
...
template <typename DType>
__global__ void roi_align(
    const unsigned char* in_data,  // (N, H, W)
    const DType* rois,  // (N, 4)
    const int num_el,
    const int height,
    const int width,
  ...
  for (size_t index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    const DType* offset_rois = rois + n * 4;
    // Do not using rounding; this implementation detail is critical
    DType roi_offset = continuous_coordinate ? static_cast<DType>(0.5) : static_cast<DType>(0);
    DType roi_start_w = offset_rois[0] - roi_offset;
    DType roi_start_h = offset_rois[1] - roi_offset;
    DType roi_end_w = offset_rois[2] - roi_offset;
    DType roi_end_h = offset_rois[3] - roi_offset;
    ...
    const unsigned char* offset_in_data =
        in_data + n * height * width;
    ...
    out_data[index] = static_cast<unsigned char>(output_val);
  }
}
...
void MRCNNMaskTargetRun<gpu>(const MRCNNMaskTargetParam& param, const std::vector<TBlob> &inputs,
  ...
  const int height =  param.image_size[0];
  const int width =   param.image_size[1];
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[mrcnn_index::kRoi].type_flag_, DType, {
    ...
    auto d_mask_t = get_1d_tensor<gpu, unsigned char>((M * M + height * width) * batch_size * num_of_rois , s, ctx);

    auto full_mask_ptr = d_mask_t.dptr_;
    auto mask_ptr = d_mask_t.dptr_ + height * width * batch_size * num_of_rois;
    ...
    int num_el = M * M * num_of_rois * batch_size;

    dim3 dimGrid = dim3(CUDA_GET_BLOCKS(num_el));
    dim3 dimBlock = dim3(kMaxThreadsPerBlock);

    roi_align<<<dimGrid, dimBlock, 0, stream>>>(full_mask_ptr, rois.dptr_,
                                     num_el, height, width, M, M,
                                     param.sample_ratio,  batch_size * num_of_rois, false,
                                     mask_ptr);

    // out: 2 * (B, N, C, MS, MS)
    const int total_num_of_masks = batch_size * num_of_rois * param.num_classes;
    write_masks_to_output_kernel<<<total_num_of_masks, 256, 0, stream>>>(mask_ptr,
                                                                         out_masks.dptr_, M,
                                                                         cls_targets.dptr_,
                                                                         param.num_classes);
